class Event < ApplicationRecord
    ...
    def record_video(f)
        ...
        if self.has_audio
            ffmpeg_command="ffmpeg -i #{capture_path} -i #{audio_path} -y -f mp4 -vcodec libx264 -preset veryslow  -crf 23 -maxrate 4M -bufsize 8M -profile:v main -acodec aac -shortest #{self.video_path(f.name)}"
        end
        puts "[ffmpeg] #{ffmpeg_command}"
        system(ffmpeg_command)
